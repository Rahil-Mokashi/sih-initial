...
preprocessing exactly (src/detection/preprocess.py's lee_filter +
normalize_db_fixed, augment=False) -- NOT the augmented train-time version,
since the point is to see what the network structurally receives, not one
...
def render_example(ax_row, image_path: Path, mask_path: Path, y: int, x: int, label: str) -> None:
    with rasterio.open(image_path) as src:
    ...
    b1_norm = normalize_db_fixed(lee_filter(b1))
    b2_norm = normalize_db_fixed(lee_filter(b2))
...
def build_montage(rows: list[tuple[Path, Path, int, int, str]], out_path: Path, title: str) -> None:
    n = len(rows)
    fig, axes = plt.subplots(n, 6, figsize=(18, 3 * n))
    if n == 1:
        axes = axes[np.newaxis, :]
    for i, (image_path, mask_path, y, x, label) in enumerate(rows):
        render_example(axes[i], image_path, mask_path, y, x, label)
...
def main() -> None:
    ...
    for label, rows_for_label in by_label.items():
        ...
        if selected:
            build_montage(selected, OUT_DIR / f"{label}_examples.png", f"{label} -- {len(selected)} real examples (train manifest)")
